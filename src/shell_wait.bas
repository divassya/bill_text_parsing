...
Public Function ShellAndWait2(ShellCommand As String, _
                    TimeOutMs As Long, _
                    ShellWindowState As VbAppWinStyle, _
                    BreakKey As ActionOnBreak) As ShellAndWaitResult
Dim TaskID As Long
...
If Trim(ShellCommand) = vbNullString Then
    ShellAndWait2 = ShellAndWaitResult.InvalidParameter
    Exit Function
End If
...
TaskID = Shell(ShellCommand, ShellWindowState)
If (Err.Number <> 0) Or (TaskID = 0) Then
    ShellAndWait2 = ShellAndWaitResult.Failure
    Exit Function
End If
...
Do Until WaitRes = WAIT_OBJECT_0
    ...
    Select Case WaitRes
        Case WAIT_ABANDONED
            ' Windows abandoned the wait
            ShellAndWait2 = ShellAndWaitResult.SysWaitAbandoned
            Exit Do
        Case WAIT_OBJECT_0
            ' Successful completion
            ShellAndWait2 = ShellAndWaitResult.Success
            Exit Do
        Case WAIT_FAILED
            ' attach failed
            ShellAndWait2 = ShellAndWaitResult.Failure
        ...
        Case WAIT_TIMEOUT
            ...
            If Ms > 0 Then
                ' user specified timeout
                If ElapsedTime > Ms Then
                    ShellAndWait2 = ShellAndWaitResult.TimeOut
                    Exit Do
                Else
                    ' user defined timeout has not expired.
                End If
...
If Err.Number = ERR_BREAK_KEY Then
    If BreakKey = ActionOnBreak.AbandonWait Then
        CloseHandle ProcHandle
        ShellAndWait2 = ShellAndWaitResult.UserWaitAbandoned
        Application.EnableCancelKey = SaveCancelKey
        Exit Function
    ElseIf BreakKey = ActionOnBreak.IgnoreBreak Then
        Err.Clear
    ...
    ElseIf BreakKey = ActionOnBreak.PromptUser Then
        ...
        If MsgRes = vbNo Then
            CloseHandle ProcHandle
            ShellAndWait2 = ShellAndWaitResult.UserBreak
            Application.EnableCancelKey = SaveCancelKey
        Else
            Err.Clear
            Resume Next
